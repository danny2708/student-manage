...
export default function AttendanceManagement() {
  ...
  const [openModal, setOpenModal] = React.useState<null | { schedule: any; mode: "take" | "edit" }>(null);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="border" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-sm text-gray-400">{availableClasses.length} found</div>
          </div>
          {loadingSchedules ? (
            <div className="text-gray-400">Loading schedules...</div>
          ) : availableClasses.length === 0 ? (
            <div className="text-gray-400">No classes scheduled for this date.</div>
          ) : (
            <div className="space-y-3">
              {availableClasses.map((s) => (
                <div key={s.id} className="flex items-center justify-between bg-gray-800 p-3 rounded">
                  <div>
                    <div className="text-sm font-semibold text-cyan-400">{s.class_name}</div>
                    <div className="text-xs text-gray-300">{s.start_time} — {s.end_time} • {s.room ?? "-"}</div>
                  </div>
                  <button
        ...
        <div className="bg-white/5 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-lg">Submitted ({selectedDate})</h3>
            <div className="text-sm text-gray-400">{submittedBySchedule.size} batches</div>
          </div>
          {loadingAttendances ? (
            ...
            <div className="space-y-3">
              {Array.from(submittedBySchedule.entries()).map(([schedule_id, records]) => {
                const schedule = schedules.find((s) => s.id === schedule_id);
                return (
                  <div key={schedule_id} className="flex items-center justify-between bg-gray-800 p-3 rounded">
                    <div>
                      <div className="text-sm font-semibold text-cyan-400">
                        {schedule?.class_name ?? `Schedule ${schedule_id}`}
                      </div>
                      <div className="text-xs text-gray-300">{records.length} records</div>
                    </div>
                    <button
                      onClick={() => {
                        if (schedule) {
      ...
      <StudentsAttendanceModal
        ...
        modalData={openModal}
        date={selectedDate}
        onSubmitted={async () => { await fetchAttendances(); }}
      />
    </div>
  );
}