...
def create_payroll(db: Session, teacher: Teacher, payroll_in: PayrollCreate):
    ...
    # Tạo bản ghi payroll (total được DB tính tự động)
    db_payroll = payroll_crud.create_payroll_record(db, payroll_in)

    # Tạo notification
    content = f"Lương tháng {db_payroll.month}/{today.year} của bạn đã được tính. Tổng lương: {db_payroll.total:.2f}"
...
def update_payroll_with_notification(db: Session, payroll_id: int, payroll_update: PayrollUpdate):
    ...
    notification_in = NotificationCreate(
        content=f"Lương tháng {db_payroll.month}/{db_payroll.sent_at.year} của bạn đã được cập nhật. Tổng lương: {db_payroll.total:.2f}",
