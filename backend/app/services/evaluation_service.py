...
def calculate_total_points_for_student(db: Session, student_id: int):
    """
...
def summarize_end_of_semester(db: Session, student_id: int):
    """
    Tổng kết điểm cuối kỳ và cập nhật điểm, giới hạn không quá 100.
    """
    total_points = calculate_total_points_for_student(db, student_id=student_id)
...
def get_summary_and_counts_for_student(db: Session, student_id: int):
    """
